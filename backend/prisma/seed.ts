...
async function main() {
  ...
  // Guest User (The "Me" user for demo)
  ...
  const listingsData = [
    {
      ...
      status: 'active', // Changed from 'available' to match new logic if needed, or keep as is if schema allows string
  ...
  // 4. Create Bookings (Past and Future for Stats)
  ...
  // Host Stats Data: Create bookings for Host User's listings from other people
  await prisma.booking.create({
    data: {
      userId: hostUser2.id, // Alex books Sofia's place
      listingId: createdListings[0].id, // Sofia's Villa
      checkIn: '2024-01-10',
      checkOut: '2024-01-15',
      totalPrice: 1250,
      guests: 4,
